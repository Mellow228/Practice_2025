...
public class ArmSubsystem extends SubsystemBase {
  ...
  public Pose3d poseA = new Pose3d(-0.26, 0, 0.2731,
      new Rotation3d(Math.toRadians(180), Math.toRadians(angle1), Math.toRadians(0)));

  public Pose3d poseB = new Pose3d(0.15, 0, 0,
      new Rotation3d(Math.toRadians(0), Math.toRadians(angle2), Math.toRadians(-0)));
  ...
  private Rectangle rectangle = new Rectangle(new Pose2d());
  ...
  public void periodic() {
    // Rectangle.updateArmAngle(angle2);
    ...
    poseA = new Pose3d(-0.26, 0, 0.2731,
        new Rotation3d(Math.toRadians(180), Math.toRadians(angle1), Math.toRadians(0)));
    poseB = new Pose3d(elbowPlace[0], elbowPlace[1], elbowPlace[2],
        new Rotation3d(Math.toRadians(0), Math.toRadians(angle2), Math.toRadians(-0)));
    Logger.recordOutput("MyPoseArray", poseA, poseB);
    Rectangle.updateArm(poseB);
  }
  ...
  public double getTargetAngle() {
    if (RobotContainer.PICKUP_STATE == RobotContainer.PickupState.HAS_NOTE && RobotContainer.ARM_STATE == RobotContainer.ArmState.AMP_STATE) {
      return -90;
    }
    return 5;
  }
  ...
  public Command printCommand(){
    return new InstantCommand(() -> System.out.println("PRINT COMMAND"));
  }
}